...
function generateAmazonPrompt(productInfo, imageType, description, styleProfile, typographyOverride) {
  ...
  const safeDescription = escapePromptText(description)
  ...
  const basePrompts = {
    ...
    scene: `Based on the reference image showing ${productName}${category ? `, a ${category}` : ''}${material ? `, made of ${material}` : ''}, create an Amazon lifestyle product photo. Place this exact product (same identity and appearance from reference) in a real, natural lifestyle scene with coherent lighting and perspective. Product is the hero element, realistic environment, premium atmosphere. Full-bleed, edge-to-edge composition: the scene must fill 100% of the canvas. No screenshot, no UI, no webpage layout, no poster mockup, no inset image, no padding, no border, no frame, no white margins, no header bar, no top title strip.${safeDescription ? ` Text content rule: the only text in the entire image must match EXACTLY character-by-character: "${safeDescription}". No paraphrase, no translation, no extension, no additional words. Single line only. Do NOT add punctuation. Do NOT add any other text anywhere in the image. Placement priority: top-left / top-right / bottom-left / bottom-right corner negative space. Avoid centered placement. Do NOT place text over the product’s main body or key detail area. Readability rule: prioritize placing text on clean negative space and choose high-contrast text color. If readability is still insufficient: first move the text to another corner; second adjust text color; third (only if needed) add a very subtle soft shadow behind the letters (opacity 8–12%, blur 6–10px). Only as the LAST resort, allow a tiny semi-transparent backing strip behind the text (NOT glassmorphism): a minimal rounded rectangle sized tightly to the text only, opacity 10–18%, no blur, no big box, and never a full-width bottom banner. Typography: ${typography}; prefer bold weight if the text is short. Text scale: about 14–24% of image height; if the text is short (<= 3 words or <= 12 characters), make it large (22–32%) and bold. Style constraints: flat text, no 3D extrude, no bevel/emboss, no outline stroke, no heavy drop shadow, no perspective warp.` : ''}`,
    
    detail: `Based on the reference image showing ${productName}${material ? `, made of ${material}` : ''}, create an Amazon product detail close-up. Extreme macro / close-up shot with tight crop: focus on the most important detail area and show fine texture and craftsmanship. The detail subject should fill 60–80% of the frame. Do NOT do wide shot, do NOT show the full product, do NOT use a lifestyle scene composition. Real-life background (tabletop, home interior, studio set) is allowed but must be strongly blurred with bokeh and shallow depth of field; keep the background minimal and unobtrusive. Premium lighting, ultra sharp focus on the detail.${safeDescription ? ` Text content rule: the only text in the entire image must match EXACTLY character-by-character: "${safeDescription}". No paraphrase, no translation, no extension, no additional words. Single line only. Do NOT add punctuation. Do NOT add any other text anywhere in the image. Placement priority: top-left / top-right / bottom-left / bottom-right corner negative space. Avoid centered placement. Never place text in the center for detail shots. Do NOT place text over the key detail area or product silhouette. Readability rule: prioritize placing text on clean negative space and choose high-contrast text color. If readability is still insufficient: first move the text to another corner; second adjust text color; third (only if needed) add a very subtle soft shadow behind the letters (opacity 8–12%, blur 6–10px). Only as the LAST resort, allow a tiny semi-transparent backing strip behind the text (NOT glassmorphism): minimal rounded rectangle sized tightly to the text only, opacity 10–18%, no blur, no big box. Never use full-width banners or large panels. Typography: ${typography}; use it as a small premium label/callout; prefer medium or bold weight; ensure readability. Text scale: about 7–11% of image height; if the text is short (<= 12 characters), make it larger (10–14%). Style constraints: flat text, no 3D extrude, no bevel/emboss, no outline stroke, no heavy drop shadow, no perspective warp.` : ''}`,
    
    aplus: `Based on the reference image showing ${productName}${category ? `, a ${category}` : ''}, create an Amazon A+ marketing poster image. Feature this exact product (same identity and appearance from reference) in a premium lifestyle scene with cohesive composition and strong marketing mood, full-frame content, no empty side panels, no split panels.${safeDescription ? ` Text content rule: the only text in the entire image must match EXACTLY character-by-character: "${safeDescription}". No paraphrase, no translation, no extension, no additional words. Single line only. Do NOT add punctuation. Do NOT add any other text anywhere in the image. Placement priority: top-left / top-right / bottom-left / bottom-right corner negative space. Avoid centered placement. Do NOT cover the product. Readability rule: prioritize placing text on clean negative space and choose high-contrast text color. If readability is still insufficient: first move the text to another corner; second adjust text color; third (only if needed) add a very subtle soft shadow behind the letters (opacity 8–12%, blur 6–10px). Only as the LAST resort, allow a tiny semi-transparent backing strip behind the text (NOT glassmorphism): minimal rounded rectangle sized tightly to the text only, opacity 10–18%, no blur, no big box, never a full-width banner. Typography: ${typography}; strong hierarchy with headline emphasis; prefer bold weight if the text is short. Text scale: about 18–30% of image height; if the text is short (<= 3 words or <= 12 characters), make it very large (26–38%) and bold. Style constraints: flat text, no 3D extrude, no bevel/emboss, no outline stroke, no heavy drop shadow, no perspective warp.` : ''}`
  }

  const prompt = basePrompts[imageType] || basePrompts.main
  if (!safeDescription || imageType === 'main') return prompt

  const textAnchor = `CRITICAL TEXT TO RENDER (EXACT, HIGHEST PRIORITY)\nTEXT_TO_RENDER_BEGIN\n${safeDescription}\nTEXT_TO_RENDER_END\nRules: render this text exactly once, character-by-character, no translation, no paraphrase, no extension, no added punctuation, single line, no other text anywhere. Typography must follow: ${typography}.\n`

  return `${textAnchor}\n${prompt}\n\nTEXT_TO_RENDER_REPEAT: ${safeDescription}`
}
